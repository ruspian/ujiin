...
import { extractDateAndTime } from "@/lib/formatDateTime";
...
export default function EditJadwalModal({
  ...
  const [selectedClasses, setSelectedClasses] = useState<string[]>(
    itemData.classes.map((c) => c.id),
  );

  const initialStart = extractDateAndTime(itemData.startTime);
  const initialEnd = extractDateAndTime(itemData.endTime);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const finalEndDateTime = `${effectiveEndDate}T${endTime}:00+08:00`;
              ...
              <div className="space-y-4 rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      name="subjectId"
                      defaultValue={itemData.subject.id}
                      required
                      className="block w-full rounded-lg border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-teal-500 focus:ring-teal-500"
                    >
                      {subjects.map((s) => (
                  ...
                  <div>
                    ...
                    <select
                      name="examTypeId"
                      defaultValue={itemData.examType.id}
                      required
                      className="block w-full rounded-lg border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-teal-500 focus:ring-teal-500"
                    >
                      {examTypes.map((e) => (
                ...
                <div>
                  ...
                  <select
                    name="academicYearId"
                    defaultValue={itemData.academicYear.id}
                    required
                    className="block w-full rounded-lg border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-teal-500 focus:ring-teal-500"
                  >
                    {academicYears.map((a) => (
              ...
              <div className="space-y-4 rounded-xl border border-orange-100 bg-orange-50/30 p-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    readOnly
                    required
                    className="block w-full rounded-lg border-gray-300 bg-gray-200 text-gray-700 px-3 py-2 text-sm font-bold cursor-not-allowed focus:ring-0 focus:border-gray-300"
                  />
                  {calculatedDuration === "" && startDate && endTime && (
                ...
                <div className="mt-4 space-y-3 border-t border-indigo-100 pt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <select
                      ...
                      defaultValue={itemData.randomizeQuestions.toString()}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <select
                      name="showResult"
                      defaultValue={itemData.showResult.toString()}
                      className="rounded-lg border-gray-300 text-xs py-1.5 pl-2 pr-8 text-gray-900 focus:border-indigo-500 focus:ring-indigo-500"
                    >
                      <option value="true">Ya</option>
                      <option value="false">Tidak</option>
